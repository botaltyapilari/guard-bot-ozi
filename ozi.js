...
client1.on("guildBanAdd", async function(guild, user) {
  ...
  guild.members.ban(entry.executor.id, {reason: "Aden System | İzinsiz kullanıcı yasaklama"}).catch(e => { })	
...
client1.on("guildMemberRemove", async kickhammer => {
  ...
  kickhammer.guild.members.ban(yetkili.id, {reason: "Aden System | izinsiz kullanıcı Kickleme!"}).catch(e => { })	
...
const entry = await member.guild
  ...
  member.guild.members.ban(member.id, {reason: "Aden System | Bot Koruma Sistemi!"}).catch(e => { })	
...
client1.on('guildUpdate', async (oldGuild, newGuild) => {
  ...
  newGuild.members.ban(entry.executor.id, {reason: "Aden System | URL Koruma Sistemi!"}).catch(e => { })	
...
client1.on("guildUpdate", async (oldGuild, newGuild) => {
  ...
  newGuild.members.ban(entry.executor.id, { reason: `Aden System | Sunucuyu izinsiz güncellemek.` }).catch(e => { })	
  const moment = require('moment');
  moment.locale('tr');
...
client2.on("roleDelete", async role => {
  ...
  role.guild.members.ban(entry.executor.id, { reason: `Aden System | İzinsiz rol silme!` }).catch(e => { })	
...
client2.on("roleCreate", async role => {
  ...
  role.delete({ reason: "Aden System | Rol Koruma Sistemi" });
...
client2.on("roleUpdate", async (oldRole, newRole) => {
  ...
  newRole.guild.members.ban(entry.executor.id, { reason: `Aden System | İzinsiz Rol Güncelleme!` }).catch(e => { })	
...
client2.on("guildMemberUpdate", async (oldMember, newMember) => {
  ...
  newMember.roles.set(oldMember.roles.cache.map(r => r.id));
    uyecik.guild.members.ban(entry.executor.id, { reason: `Aden System | İzinsiz Yönetici Verme!` }).catch(e => { })	
...
client3.on("channelDelete", async channel => {
  ...
  channel.guild.members.ban(entry.executor.id, { reason: `Aden System | İzinsiz Kanal Silme!` }).catch(e => { })	
  await channel.clone({ reason: "Aden System | Kanal Koruma Sistemi!" }).then(async kanal => {
...
client3.on("channelCreate", async channel => {
  ...
  channel.guild.members.ban(entry.executor.id, { reason: `Aden System | İzinsiz Kanal Oluşturma!` }).catch(e => { })
  channel.delete({reason: "Aden System | Kanal Koruma Sistemi!"}).catch(e => { })
...
client3.on("channelUpdate", async (oldChannel, newChannel) => {
  ...
  newChannel.guild.members.ban(entry.executor.id, { reason: `Aden System | İzinsiz Kanal Güncellemek!` }).catch(e => { })
...
client3.on("webhookUpdate", async (channel) => {
  ...
  await webhooks.map(x => x.delete({reason: "Aden System | Webhook Silindi!"}))
